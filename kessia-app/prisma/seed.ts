...
async function main() {
  ...
  const mkt = await prisma.$transaction(
    [
      { seller: akossiwa, title: 'Congélateur horizontal 200 L', description: 'Bon état, très peu servi. Idéal boutique ou revente de produits surgelés. Retrait à Lomé.', category: 'EQUIPEMENT', price: 145_000, city: 'Lomé', pickupZone: 'tokoin', payableByTontine: true, tontineInstallments: 5, days: 2 },
      { seller: kossi, title: 'Onduleur 1200 VA + 2 batteries', description: 'Protège votre matériel des coupures. Batteries changées il y a 6 mois.', category: 'EQUIPEMENT', price: 90_000, city: 'Lomé', pickupZone: 'be', payableByTontine: true, tontineInstallments: 4, days: 5 },
      ...
      { seller: afiwa, title: 'Machine à coudre industrielle', description: 'Piqueuse plate, moteur silencieux. Parfaite pour un atelier de couture.', category: 'EQUIPEMENT', price: 175_000, city: 'Lomé', pickupZone: 'adidogome', payableByTontine: true, tontineInstallments: 6, days: 3 },
  ...
  const seedOrder = await prisma.marketplaceOrder.create({
    data: {
  ...
  // …avec une livraison Miaride simulée en cours (ADR 0042)
  await prisma.marketplaceDelivery.create({
    data: {
      orderId: seedOrder.id, buyerId: kossi.id, provider: 'MIARIDE', mode: 'SIMULATED',
      status: 'IN_TRANSIT', pickupLabel: 'Agoè-Nyivé · Lomé', dropoffAddress: 'Nyékonakpoè, près de la station',
      dropoffArea: 'nyekonakpoe', recipientPhone: kossi.phone, feeAmount: dec(850), feeCurrency: 'XOF',
      feeLedgerRef: `DELIV-${seedOrder.id}`, etaMinutes: 30, providerRef: 'MIA-K7P2QX',
